// MAIN SETUP SCREEN - where users configure event settings before starting

import { AnimatePresence, motion } from "framer-motion";
import { useState } from "react";
import { ICE_BREAKERS } from "../constants/icebreakers";
import type { Config } from "../types";
import { playStartEvent, unlockAudio } from "../utils/audio";
...
export function SetupScreen({ onStart }: { onStart: (cfg: Config) => void }) {
  ...
  return (
    <motion.div
      ...
      className="flex-1 w-full flex flex-col items-center justify-center p-6 overflow-auto min-h-0"
      style={{
        ...
        {showPreview && (
          <motion.div
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl h-full max-h-[85vh] flex flex-col rounded-3xl overflow-y-auto"
              style={{
              ...
              <div className="shrink-0 p-8 border-b border-white/5 flex justify-between items-center">
                <div>
                  <h2
                    className="text-3xl font-black tracking-tight"
                    style={{ fontFamily: "'Instrument Sans', sans-serif", color: "#fff" }}>
                    Icebreakers Preview
                  </h2>
                  <p
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-3 custom-scrollbar">
                {ICE_BREAKERS.map((q, i) => (
                  <motion.div
                    ...
                    <span
                      className="text-[#f97316] text-xs font-bold mr-4"
                      style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <span
                      className="text-lg text-white/90 font-medium tracking-wide group-hover:text-white transition-colors"
      ...
      <div
        className="w-full max-w-lg relative z-10 transition-all flex flex-col items-center"
        style={{ filter: showPreview ? "blur(8px)" : "none", opacity: showPreview ? 0.3 : 1 }}>
        ...
        <motion.div
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={click(() => setRounds(Math.max(1, rounds - 1)))}
                className="w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold transition-all active:scale-90"
                style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#f97316" }}>
                −
              </button>
              <div
                className="flex-1 text-center text-5xl font-black"
                style={{ fontFamily: "'Instrument Sans', sans-serif", color: "#fff" }}>
                {rounds}
              </div>
              <button
                onClick={click(() => setRounds(Math.min(20, rounds + 1)))}
                className="w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold transition-all active:scale-90"
                style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#f97316" }}>
                +
              </button>
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {[15, 30, 45, 60, 90].map((s) => (
                <button
                  key={s}
                  onClick={click(() => setBreakDuration(s))}
                  className="px-5 py-3 rounded-xl text-sm font-bold transition-all active:scale-95"
                  style={{
                    fontFamily: "'JetBrains Mono', monospace",
        ...
        <motion.div
          ...
          <button
            ...
            className="w-100 py-5 rounded-2xl text-lg font-black tracking-wider uppercase transition-all active:scale-[0.98]"
            style={{
              fontFamily: "'Instrument Sans', sans-serif",
      ...
      <button
        ...
        className="mt-8 px-5 py-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-white/60 hover:text-[#f97316] hover:border-[#f97316]/50 transition-all cursor-pointer backdrop-blur-md flex items-center gap-3"
